...
public class AccountQuartzListener implements ServletContextListener {
	...
	public void contextInitialized(ServletContextEvent sce) {
		...
		if ( springContext != null ) {
						try {
							QuartzManager.addJob(QuartzManager.getsched(), "预算表生成消息提醒任务定时器", QuartzBudget.class, "0 03 16 * * ?",null);//每年一月份一号生成预算表
							
								//QuartzManager.addJob(QuartzManager.getsched(), "预算表生成消息提醒任务定时器", QuartzBudget.class, "0 0 0 1 1 ?",null);//每年一月份一号生成预算表
								QuartzManager.addJob(QuartzManager.getsched(), "损益表生成消息提醒任务定时器", QuartzJob.class, "0 43 16 * * ?",null);//每个月11号生成损益表
