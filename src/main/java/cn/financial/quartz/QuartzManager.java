...
public class QuartzManager {
	...
	public static void addJob(Scheduler sched, String jobName,
			...
			JSONObject jsonb) {
		try {
			CronTrigger trigger1 = (CronTrigger) sched.getTrigger(jobName,
					TRIGGER_GROUP_NAME);
			if (trigger1 == null) {
				...
				sched.resumeAll();// 恢复调度中所有的job的任务
				// 启动
				if (!sched.isShutdown()) {
					sched.start();
				}
			} else {
				modifyJobTime(sched, jobName, time, jsonb, cls);
			}
		} catch (Exception e) {
	...
	public static void addJob(Scheduler sched, String jobName,
		...
		try {
			...
			jobDetail.getJobDataMap().put("quartz", jsonb);
			// 触发器
	...
	public static void modifyJobTime(Scheduler sched, String jobName,
			String time, JSONObject jsonb,
			@SuppressWarnings("rawtypes") Class jobClass) {
		try {
			CronTrigger trigger = (CronTrigger) sched.getTrigger(jobName,
					TRIGGER_GROUP_NAME);
			if (trigger == null) {
				addJob(sched, jobName, jobClass, time, jsonb);
				return;
			}
			String oldTime = trigger.getCronExpression();
			JobDataMap map = trigger.getJobDataMap();
			if (!oldTime.equalsIgnoreCase(time) || jsonb.getInt("state") == 1
					|| map.getInt("id") != jsonb.getInt("id")) {
	...
	public static void modifyJobTime(Scheduler sched, String triggerName,
			String triggerGroupName, String time, JSONObject jsonb,
			@SuppressWarnings("rawtypes") Class jobClass) {
		try {
			CronTrigger trigger = (CronTrigger) sched.getTrigger(triggerName,
					triggerGroupName);
			if (trigger == null) {
				addJob(sched, triggerName, triggerGroupName, triggerName,
						triggerGroupName, jobClass, time, jsonb);
				return;
			}
			String oldTime = trigger.getCronExpression();
